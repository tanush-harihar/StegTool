...
void WAVCarrier::embed(const ByteArray& data) {
    ...
    // Calculate space needed: 8-byte length prefix + data
    size_t total_embedded = 8 + data.size();
    if (total_embedded > max_capacity) {
        throw InsufficientCapacityException(total_embedded, max_capacity);
    }

    // Build the full embedded stream: length prefix (big-endian) + data
    ByteArray embedded;
    embedded.reserve(8 + data.size());
    for (int i = 7; i >= 0; --i) {
        embedded.push_back(static_cast<uint8_t>((data.size() >> (i * 8)) & 0xFF));
    }
    embedded.insert(embedded.end(), data.begin(), data.end());

    size_t data_bit_index = 0;
    for (size_t i = 0; i < audio_data_.size() && data_bit_index < embedded.size() * 8; ++i) {
        uint8_t bit_to_embed = (embedded[data_bit_index / 8] >> (7 - (data_bit_index % 8))) & 1;
...
ByteArray WAVCarrier::extract() {
    // First, extract the 8-byte length prefix
    ByteArray length_bytes(8);
    size_t bit_index = 0;
    for (size_t i = 0; i < audio_data_.size() && bit_index < 64; ++i) {
        uint8_t bit = audio_data_[i] & 1;
        length_bytes[bit_index / 8] |= static_cast<uint8_t>(bit << (7 - (bit_index % 8)));
        ++bit_index;
    }

    // Read length
    uint64_t embedded_len = 0;
    for (int i = 0; i < 8; ++i) {
        embedded_len = (embedded_len << 8) | static_cast<uint64_t>(length_bytes[i]);
    }

    // Sanity check: avoid buffer overflow
    if (embedded_len > capacity() || embedded_len == 0) {
        return {}; // No valid embedded data
    }

    // Extract the actual data
    ByteArray extracted(embedded_len);
    size_t data_bit_index = 0;
    size_t bytes_read = 0;
    for (size_t i = 0; i < audio_data_.size() && bytes_read < 8 + embedded_len; ++i) {
        // Skip first 64 bits (length prefix)
        if (bit_index < 64) {
            ++bit_index;
            continue;
        }
        ...
        uint8_t bit = audio_data_[i] & 1;
        extracted[(bit_index - 64) / 8] |= static_cast<uint8_t>(bit << (7 - ((bit_index - 64) % 8)));
        ++bit_index;
